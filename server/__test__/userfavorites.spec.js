...
describe('Tests for Favorites API endpoint', () => {
  describe('#Handle addition of user favorites', () => {
    ...
    it('Should return a 403 when a user who is not authenticated attempts to favorite a recipe', (done) => {
      request
        .post(`${recipesApi}/${recipe[0].id}/favorites`)
    ...
    it('Should return a 404 for an invalid route', (done) => {
      request
        .post(`${recipesApi}/${recipe[0].id}/favorite`)
        .set('Connection', 'keep alive')
        .set('Content-Type', 'application/json')
        .type('form')
        .end((err, res) => {
    ...
    it('Should successfuly favorite a recipe by an authenticated user', (done) => {
      ...
      request
        .post(`${recipesApi}/${recipe[0].id}/favorites?token=${
          testUser.tokens[0].token
        }`)
        .set('Connection', 'keep alive')
        .set('Content-Type', 'application/json')
        .type('form')
        .end((err, res) => {
          ...
          expect(res.body.message).to.equal('Recipe added to your list of favorites.');
          done();
        });
    });
    it('Should return the favorites of a logged in user', (done) => {
      const testUser = { ...users[1] };
      request
        .get(`${favoriteApi}/${testUser.id}/favorites?token=${
          testUser.tokens[0].token
        }`)
        .set('Connection', 'keep alive')
        .set('Content-Type', 'application/json')
    ...
    it('Should return a 404 if the recipe does not exist', (done) => {
      request
        .post(`${recipesApi}/100/favorites?token=${users[0].tokens[0].token}`)
        .set('Connection', 'keep alive')
        .set('Content-Type', 'application/json')
        .type('form')
        .end((err, res) => {
    ...
    it('Should prevent a user from favoriting his own recipe', (done) => {
      ...
      request
        .post(`${recipesApi}/${testRecipe.id}/favorites?token=${
          testUser.tokens[0].token
        }`)
        .set('Connection', 'keep alive')
        .set('Content-Type', 'application.json')
