...
describe('Recipes Endpoint', () => {
  describe('#Test Creating a new Recipe', () => {
    ...
    it('Should successfully create a recipe as a logged in user', (done) => {
      ...
      request
        .post(`${recipesApi}?token=${testUser.tokens[0].token}`)
        .set('Connection', 'keep alive')
        .set('Content-Type', 'application/json')
        .type('form')
        .send(recipes[1])
    ...
    it('Should return a 400 if no ingredients are provided on recipe creation', (done) => {
      ...
      request
        ...
        .end((err, res) => {
          expect(res.status).to.equal(400);
          expect(res.body.message).to.equal('Input the ingredients for your recipe.');
    ...
    it('Should return a 403 if a user tries to edit a recipe without a token', (done) => {
      ...
      request
        .put(`${recipesApi}/${recipeId}/modify`)
        .set('Connection', 'keep alive')
        .set('Content-Type', 'application/json')
        .type('form')
        .send(testRecipe)
    ...
    it('Should return a 404 if the recipe to be edited is not found', (done) => {
      ...
      request
        ...
        .end((err, res) => {
          ...
          expect(res.body.message).to.equal('It seems this recipe does not exist.');
          done();
        });
    });
    it('Should prevent an unauthenticated user from deleting a recipe', (done) => {
      request
        .delete(`${recipesApi}/${recipes[0].id}`)
        .set('Connection', 'keep alive')
        .set('Content-Type', 'application/json')
        .type('form')
        .end((err, res) => {
    ...
    it('Should prevent a user from deleting a recipe they do not own', (done) => {
      const testUser = { ...users[0] };
      request
        .delete(`${recipesApi}/${recipes[0].id}?token=${testUser.tokens[0].token}`)
        .set('Connection', 'keep alive')
        .set('Content-Type', 'application/json')
        .type('form')
        .end((err, res) => {
    ...
    it('Should return a 404 if the recipe to be deleted does not exist', (done) => {
      const testUser = { ...users[0] };
      request
        .delete(`${recipesApi}/${recipes[1].id}?token=${testUser.tokens[0].token}`)
        .set('Connection', 'keep alive')
        .set('Content-Type', 'application/json')
        .type('form')
        .end((err, res) => {
    ...
    it('Should prevent a non-logged in user from posting a review on a recipe', (done) => {
      request
        .post(`${recipesApi}/${recipes[0].id}/reviews`)
        .set('Connection', 'keep alive')
        .set('Content-Type', 'application/json')
        .type('form')
        .send(reviews)
    ...
    it('Should prevent a user from posting an empty review', (done) => {
      ...
      request
        .post(`${recipesApi}/${recipes[0].id}/reviews?token=${
    ...
    it('Should add a review by an authenticated user', (done) => {
      request
        ...
        .end((err, res) => {
          ...
          expect(res.body.reviewData.review).to.be.a('string');
          done();
        });
    });
    it('Should fetch a single recipe from the application', (done) => {
      request
        .get(`${recipesApi}/${recipes[0].id}`)
        .set('Connection', 'keep alive')
        .set('Content-Type', 'application/json')
        .type('form')
        .end((err, res) => {
    ...
    it("Should return all the user's recipe in the application", (done) => {
      const testUser = { ...users[0] };
      request
        .get(`/api/v1/users/recipes?token=${testUser.tokens[0].token}`)
        .set('Connection', 'keep alive')
        .set('Content-Type', 'application/json')
        .type('form')
        .end((err, res) => {
          expect(res.status).to.equal(200);
          expect(res.body.message).to.equal(`You currently have ${res.body.userRecipe.length} recipe(s)`);
