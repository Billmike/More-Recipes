    ...
    Recipes.findById(req.params.recipeId)
      .then((foundRecipe) => {
        ...
        return Votes.findAll({
          ...
          const votersArray = [];
          votedRecipes.forEach(elem =>
            votersArray.push(elem.dataValues.userId));

          if (votersArray.includes(req.userId)) {
            const checkVoteType = votedRecipes.filter(elem => elem.dataValues.userId === req.userId)[0];
            if (checkVoteType.dataValues.voteType === req.params.vote) {
              res.status(200).json({
                message: `Recipe removed from your list of ${
                  req.params.vote
                }d recipes`
              });
            ...
            return Votes.findById(checkVoteType.dataValues.id)
              .then(updateVote =>
                updateVote.update({ voteType: req.params.vote }))
              .then(() =>
                res.status(200).json({
                  message: `${req.params.vote} successful.`
                }));
